"""MVP 제안서 양식 pptx에 마중(Majung) 내용을 채워 제출본을 생성.
...
def main():
    ...
    tb = add_text(s[0], 2.9, 4.0, 7.5, 0.8, [
        ("마중 (Majung)", 18, NAVY, True),
        ("외국인 노동자 맞춤형 위임 뱅킹 에이전트 · 전북은행·브라보코리아·한패스 연계", 11, BLACK, False),
    ])
    for para in tb.text_frame.paragraphs:
        para.alignment = PP_ALIGN.CENTER
    fill_form_table(s[0], {
        "팀명": [("마중", 11, BLACK, True)],
        "서비스명": [("마중 (Majung)", 11, NAVY, True)],
    })
    ...
    fill_form_table(s[1], {
        ...
        "서비스명": [
            ("마중 (Majung)", 13, NAVY, True),
            ("외국인 근로자 특화 위임(Mandate)형 뱅킹 에이전트", 10.5, BLACK, False),
        ],
        "주제": [
            ("자유주제: 외국인 근로자 신용 사다리 구축을 위한 뱅킹 에이전트 (전북은행·브라보코리아·한패스 연계)",
             11, BLACK, False),
        ],
        "한줄소개": [
            ...
            ("모국어 한마디로 송금을 위임하고, 사채 대환 안내를 통해 전북은행의 우량 고객으로 편입시키는 AI 에이전트",
             10.5, BLACK, False),
        ],
    })
    ...
    add_text(s[2], 0.95, 1.30, 11.4, 0.4,
             [("입국 전 발생한 고금리 사채가 외국인 고객의 제도권 금융 진입을 원천 차단", 13.5, NAVY, True)])
    journey = [
    ...
    add_text(s[2], 0.95, 4.28, 11.4, 1.1, [
        ...
        ("· 시사점: 선제적 록인(Lock-in) 및 방어 전략 부재 시 핵심 우량 고객 대규모 이탈 불가피", 10.5, BLACK, False),
    ])
    add_text(s[2], 0.95, 5.50, 11.4, 0.95, [
        ("[전략] 신규 유치(CAC) 출혈경쟁을 탈피한 '대환 중심의 질적 성장'", 11, NAVY, True),
        ("· 핵심 KPI: 고객 생애 가치(LTV) 제고 및 타행/사채 대환 전환율", 10.5, BLACK, False),
    ...
    add_box(s[3], 0.95, 2.2, 5.55, 0.95, [
        ("Step 1. 위임 송금을 통한 고객 록인 (유입 채널)", 11, NAVY, True),
    ...
    add_box(s[3], 0.85, 3.5, 1.75, 1.05,
            [("발화 및 급여 입금", 10, BLACK, True), ("트리거 이벤트", 9, BLACK, False)], RGBColor(0xF2, 0xF2, 0xF2), GRAY)
    add_arrow(s[3], 2.62, 3.95, 0.33)
    add_box(s[3], 2.97, 3.5, 1.95, 1.05,
            [("LLM 의도 파싱", 10, BLACK, True), ("자연어 해석 전담", 8.5, BLACK, False),
             ("실행 권한 없음", 8.5, BLACK, False)],
            RGBColor(0xF2, 0xF2, 0xF2), GRAY)
    add_arrow(s[3], 4.94, 3.95, 0.33)
    add_box(s[3], 5.29, 3.5, 4.55, 1.08, [("3중 보안 게이트 (결정적 코드 기반 판정)", 9.5, NAVY, True)],
            LBLUE, NAVY, anchor=MSO_ANCHOR.TOP)
    for gx, ttl, det in [(5.42, "Gate A", "위임 검증"), (6.85, "Gate B", "한도·환율"), (8.28, "Gate C", "수취인·AML")]:
    ...
    add_text(s[3], 0.95, 4.8, 11.4, 1.6, [
        ("[제1원칙] 금융 사고 원천 차단을 위한 LLM 실행 권한 배제", 12, NAVY, True),
        ("· LLM은 고객의 자연어 의도 파싱 및 번역만 수행하며, 실제 송금액 산정과 실행 여부는 내부 '결정적 코드(Rule)'가 통제", 11, BLACK, False),
        ("[포지셔닝] 브라보코리아 앱 내장형 대환 전환 엔진", 12, NAVY, True),
    ...
    add_table(s[4], 0.95, 2.12, 11.4, 1.5, [
        ["외국인 근로자 핵심 과업 (JTBD)", "마중(Majung) 솔루션 (→ §3 기능 명세 연계)"],
        ["\"환율이 유리할 때 송금하고 싶지만 일하느라 매번 확인하기 어렵다\"", "급여 입금 트리거 + 목표 환율 도달 시 자동 송금 실행"],
        ["\"잘못 송금되거나 사기를 당할까 봐 두렵다\"", "3중 게이트 기반 자동 보류 및 자금세탁방지(AML) 선차단"],
    ...
    loop = [
        ([("판단 (Decision)", 11, WHITE, True), ("Gate A·B·C 검증", 8.5, WHITE, False)], NAVY, NAVY),
        ([("행동 (Action)", 11, WHITE, True), ("실행 / 보류 / 모국어 알림", 8.5, WHITE, False)], BLUE, NAVY),
    ...
    add_text(s[4], 0.95, 5.16, 11.4, 1.3, [
        ("▶ 세부 파이프라인 (평가 6단계): 수집 → 검색 → 판단 → 생성 → 검증 → 후속액션", 11, NAVY, True),
        ("· 기능 구현 완료: 위임장 발급, 조건부 자동 송금, 의심 거래 보류, 사기 거래 차단, 대환대출 회부 (단위 테스트 20건 전원 통과)", 11, BLACK, False),
        ("· 시스템 설계 경계: 대출 최종 승인 권한은 전북은행 고유 엔진에 위임, 단일 사고 발생 시 최대 손실액을 위임 한도 내로 캡핑(Capping)",
         11, BLACK, False),
    ])
    ...
    add_text(s[5], 0.95, 2.18, 11.4, 1.5, [
        ("· LLM: 모델 추상화 계층으로 상용 LLM(Claude 등) 연동 설계, 의도 파싱·번역 전담. 예선 PoC는 룰 기반 폴백(본선 무변경 치환), RAG 미적용", 11, BLACK, False),
        ("  계좌·금액·실명은 미전송하고 비식별 의도 텍스트만 전달, 판정은 SQLite 결정적 코드 전담. 상용화 시 국내 리전 또는 행내 sLLM 전환 가능", 10, GRAY, False),
        ("· 프라이버시 보호: 외부 LLM 호출 시 민감 식별정보 토큰화(비식별화) 처리, 판정 및 감사 로그는 내부 망에서 단독 처리", 11, BLACK, False),
    ...
    add_table(s[5], 0.95, 4.12, 11.4, 1.95, [
        ["예상되는 기술적 제약", "마중(Majung)의 대응 전략"],
        ["코어뱅킹 실연동 제약 (망분리 및 보안성 심의)", "모의 API 기반 PoC 선행 구현, 실제 코어 연동은 서비스 상용화 단계로 분리 설계"],
        ["LLM 환각(Hallucination)에 따른 금융 사고 책임", "LLM의 송금 실행 권한 박탈, 송금처 화이트리스트 관리 및 1회 송금 한도 캡핑"],
        ["환율 예측의 불확실성", "단순 예측 배제, '7일 이동평균 대비 1% 이상 시' 등 명시적 Rule 적용 및 환율 데이터 60초 지연 시 거래 정지"],
        ["외국인 대상 자동화 송금에 따른 자금세탁(AML) 리스크", "Gate C 내부 스코어링 적용, 이상 징후 시 의심거래보고(STR) 큐 적재 및 송금 선차단"],
    ], col_widths=[4.4, 7.0], fsize=10.5)

    # ════════ Slide 6 · 사용자 데모 시나리오 ════════
    remove_guidance(s[6]); remove_frame_title(s[6])
    add_text(s[6], 0.95, 1.30, 11.4, 0.4,
             [("입국부터 대환까지 이어지는 End-to-End 시나리오 검증 완료", 13.5, NAVY, True)])
    ...
    inserted = add_capture_row(
        ...
        caption="실동작 화면: ② 조건부 자동 송금(철회 보장) / ③ 이상 거래 보류(Score 75) / ⑤ 대환대출 제안(투명한 정보 고지)")
    if not inserted:
    ...
    add_text(s[7], 6.55, 1.88, 6.05, 2.85, [
        ...
        ("[업무효율] 대면 모집 비용 없이 고객획득비용(CAC) 최소화, 플랫폼 연계 수수료만 발생", 11, BLACK, False),
        ("[리스크감소] AML 자동 탐지(스코어 40·70 이중 컷)와 모국어 위임장 재확인으로 불완전판매·STR 누락 위험 제거", 11, BLACK, False),
        ("[전략 연계] JB금융그룹 AI 전환(AX) 및 외국인·전략 여신 확대 방침과 직접 연계, 급여 집중으로 주거래 록인", 11, NAVY, True),
    ])
    add_table(s[7], 0.95, 4.78, 11.4, 1.2, [
        ["유관 부서", "예상되는 내부 허들 및 우려 사항", "마중(Majung)의 전략적 방어 논리"],
    ...
    add_text(s[7], 0.95, 6.06, 11.4, 0.3, [
        ("[향후 확장성] 17개국 다국어 지원, 음성 인식(STT) 고도화, 시니어/소상공인 대상 서비스 확장, 코어뱅킹 SI 실연동", 9.5, GRAY, False),
    ])
    src(s[7], "출처: 전북은행 점유율 72% 및 1.3조 목표치(JB금융지주 IR 컨퍼런스콜, 2025) / 대환잔액 및 수익 지표는 보수적 가정을 적용한 자체 추정치 (산출 근거 부록 참조)")
