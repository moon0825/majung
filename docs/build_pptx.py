...
def add_quant_chart(slide, l, t, w, h):
    ...
    cd.categories = ["보수(3%)", "기준(7%)", "낙관(12%)"]
    cd.add_series("대환 잔액(억 원)", (216, 504, 864))
    cd.add_series("연 이자수익(억 원)", (20.7, 48.3, 82.9))
...
def main():
    ...
    add_text(s[3], 0.95, 4.8, 11.4, 1.6, [
        ...
        ("· 브라보코리아(전북은행 자체 외국인 플랫폼) 기존 외국인 고객 24만 명 기반, 대환대출·적금으로 연결", 11, BLACK, False),
    ])
    ...
    add_text(s[7], 0.95, 1.30, 11.4, 0.4,
             [("대환 전환율 7% 달성 시, 연간 이자수익 약 48억 원 창출 전망 (자체 추정치)", 13.5, NAVY, True)])
    add_quant_chart(s[7], 0.8, 1.88, 5.5, 2.78)
    add_text(s[7], 6.55, 1.88, 6.05, 2.85, [
        ("[수익기회] 대환 잔액 504억 원 = 모수 4.8만 명 × 전환율 7% × 인당 1,500만 (자체 추정)", 11, BLACK, False),
        ("[수익기회] 연 이자수익 약 48억 원 = 504억 × 금리 스프레드 9.59%p (대손충당금·운영비 차감 전)", 11, BLACK, False),
        ("[고객가치] 1인당 연간 약 246만 원 이자 비용 절감 (1,500만 × 16.41%p)", 11, BLACK, False),
